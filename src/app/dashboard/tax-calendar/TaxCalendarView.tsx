...
export function TaxCalendarView({ initialDeadlines }: { initialDeadlines: TaxDeadline[] }) {
    ...
    const { isLoaded: roleLoaded } = useRoles();
    ...
    const toggleStatus = async (id: string, newStatus: TaxDeadline["status"]) => {
        ...
        if (res.success) {
            ...
            alert(res.error || "Gagal mengubah status deadline");
        }
    };
